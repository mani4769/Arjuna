...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail(mailOptions);
    res.json({ message: 'Login successful, email sent' });
  } catch (error) {
...
app.put('/updateTeam/:email', async (req, res) => {
  ...
  try {
    ...
    console.error('Error in /updateTeam/:email route:', error);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});
...
app.delete('/deleteTeam/:email', async (req, res) => {
  ...
  try {
    ...
    console.error('Error in /deleteTeam/:email route:', error);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});
...
app.get('/api/schedule', async (req, res) => {
  try {
    ...
    console.error('Error in /api/schedule route:', error);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});
...
app.post('/api/alert', async (req, res) => {
  ...
  try {
    ...
    console.error('Error in /api/alert route:', error);
    res.status(500).json({ message: 'Failed to schedule alert', error: error.message });
  }
});
...
app.post('/verifypassword', async (req, res) => {
  ...
  try {
    ...
    console.error('Error in /verifypassword route:', error);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});
...
app.post('/api/scorers', async (req, res) => {
  ...
  try {
    const newScorer = new Scorer({ name, email });
    await newScorer.save();
    res.status(201).json({ message: 'Scorer added successfully' });
  } catch (error) {
    console.error('Error in /api/scorers route:', error);
    res.status(500).json({ message: 'Failed to add scorer', error: error.message });
  }
});
...
app.post('/api/umpires', async (req, res) => {
  ...
  try {
    ...
    console.error('Error in /api/umpires route:', error);
    res.status(500).json({ message: 'Failed to add umpire', error: error.message });
  }
});
...
app.post('/api/venues', async (req, res) => {
  ...
  try {
    ...
    console.error('Error in /api/venues route:', error);
...
app.post('/api/match-schedule', async (req, res) => {
  ...
  try {
    const newMatchSchedule = new MatchSchedule({ team1, team1Players, team2, team2Players, scorerName, umpireName, venueName, dateTime });
    await newMatchSchedule.save();
    res.status(201).json({ message: 'Match schedule created successfully' });
  } catch (error) {
    console.error('Error in /api/match-schedule route:', error);
    res.status(500).json({ message: 'Failed to create match schedule', error: error.message });
  }
});
